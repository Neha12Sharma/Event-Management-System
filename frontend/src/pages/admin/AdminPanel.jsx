...
import {
    mockGetEvents, mockCreateEvent, mockUpdateEvent,
    mockDeleteEvent, INITIAL_CATEGORIES
} from '../../api/mockData';
...
export default function AdminPanel() {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (editing === 'new') {
                ...
                toast('Event created! ✅ (Demo Mode)', 'success');
            } else {
                const updatedEv = mockUpdateEvent(editing, form);
                setEvents(evs => evs.map(ev => ev.id === editing ? (updatedEv || ev) : ev));
                toast('Event updated! ✅ (Demo Mode)', 'success');
            }
            setEditing(null);
        } finally {
            setSaving(false);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            mockDeleteEvent(id);
            setEvents(evs => evs.filter(ev => ev.id !== id));
            toast('Event deleted (Demo Mode)', 'info');
        }
    };
